...
public class ForcepsAgent<C extends AxialSpring> extends AddComponentAgent<C> {
   ...
   protected void initializePrototype (ModelComponent comp, Class type) {
      ...
      } else if (type == Forceps.class) {
         Forceps axial = (Forceps)comp;
         RenderProps.setLineRadius (axial, getDefaultLineRadius()*0.001);
         axial.setMaterial (null);
   ...
   public ForcepsAgent(Main main, VirtualHipFractureSurgery sim) {
      this(main, (ComponentList<C>)sim.getMechMod ().axialSprings(), sim.getMechMod ());
      myModel = sim.getMechMod ();
      this.sim = sim;  
   }

   public ForcepsAgent (Main main, ComponentList<C> list,
   ...
   protected void createDisplay() {
      createDisplayFrame ("Add Forceps");
   ...
   private void createAndAddSpring() {
      ...
      try {
         spring = (Forceps)myComponentType.newInstance();
      }
      catch (Exception e) {
      ...
      RenderProps.setLineColor (spring, Color.decode ("#C0C0C0"));

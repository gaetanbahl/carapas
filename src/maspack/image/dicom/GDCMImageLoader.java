...
public class GDCMImageLoader {

   public static class MyWatcher extends SimpleSubjectWatcher {
      public MyWatcher(Subject s) {
         super(s, "Override String");
      }
   ...
   public static boolean WritePNG(Bitmap input, String outfilename) {
      ...
      if (pf.GetSamplesPerPixel() == 1) {
         if (pi.GetType() == PhotometricInterpretation.PIType.MONOCHROME1
            ...
            .GetType() == PhotometricInterpretation.PIType.PALETTE_COLOR) {
      ...
      if (pi.GetType() == PhotometricInterpretation.PIType.PALETTE_COLOR) {
         bi =
            new BufferedImage(
               colorModel, colorModel.createCompatibleWritableRaster(
   ...
   public static void main(String[] args) throws Exception {
      ...
      System.load("D:\\school\\workspace\\artisynth_core\\lib\\Windows64\\libgdcmjni.dll");
      
      String directory = "D:\\school\\workspace\\artisynth_projects\\src\\artisynth\\models\\dartmouth\\data\\PatCase5_CTs_Dicom\\Post-deform";
      ...
      Tag[] tagarray =
         { new Tag(0x0010, 0x0010), // PatientName
           ...
           new Tag(0x0018, 0x0050), // SliceThickness ?? Eg. Enhanced MR Image
                                    // Storage
      ...
      for (long idx = 0; idx < fns.size() && idx < 1; ++idx) {
         Reader r = new Reader();
...
//         b = r.ReadUpToTag(new Tag(0x88, 0x200), tst);
         ...
         Tag tag = new Tag(0x0002, 0x0000);
         DataElement elem = r.GetFile().GetDataSet().FindNextDataElement(tag);
         StringFilter sfilter = new StringFilter();
         sfilter.SetFile(r.GetFile());
         while (elem != null) {
            System.out.println(sfilter.ToStringPair(elem).getFirst() + "," + sfilter.ToStringPair(elem).getSecond());
            Value val = elem.GetValue();
            VR vr = elem.GetVR();

            tag = elem.GetTag();
            tag.SetElement(tag.GetElement()+1);
            System.out.println(Integer.toHexString(tag.GetGroup()) + "," + Integer.toHexString(tag.GetElement())); 
            elem = r.GetFile().GetDataSet().FindNextDataElement(tag);
         }

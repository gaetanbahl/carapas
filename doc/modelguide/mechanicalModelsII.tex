...
\ref{ContactImplementation:sec}). If not specified, the system will
...
Section \ref{ContactImplementation:sec}.
...
\label{ContactImplementation:sec}
...
meshes must (at present) be triangular, closed, and manifold.  A
bounding-box hierarchy is used to determine which surfaces meshes
might intersect and then locate the individual triangle
intersections. What happens next then depends on the setting of the
collision manager's {\sf colliderType} property: \javaclassAlt{%
artisynth.core.mechmodels.CollisionManager\$ColliderType\#TRI\_INTERSECTION}%
{TRI\_INTERSECTION} (the current default) groups the triangle
intersections into contact regions mesh for purposes of finding
interpenetrating vertices, while \javaclassAlt{%
artisynth.core.mechmodels.CollisionManager\$ColliderType\#TRI\_INTERSECTION}%
{AJL\_CONTOUR} also uses the intersections to compute the (piecewise
linear) intersection contours between the meshes. This information is
then used to create a set of contact points, normals and associated
constraints that are used to resolve the collision, as shown in
Figure~\ref{Collision:fig}.
...
How the contact constraints are actually generated depends on the
collision behavior's {\sf method} property, as mentioned in Section
\ref{collisionBehavior:sec}. The standard methods are:
